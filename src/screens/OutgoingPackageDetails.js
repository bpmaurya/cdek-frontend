...
import { outgoingPackageDetails,updateOutgoingPackage } from "../actions/outgoingPackageAction";
...
function OutgoingPackageDetails({ match, history }) {
  ...
  const updateOutgoing = useSelector(
    (state) => state.updateOutgoing
  );
  const { error:updateError, loading:updateLoading,success:updateSuccess } = updateOutgoing;
